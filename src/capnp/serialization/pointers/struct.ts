...
import { getByteLength, getDataWordLength, getWordLength, ObjectSize } from '../object-size.ts';
import { Segment } from '../segment.ts';
import { Data } from './data.ts';
import { List, ListCtor } from './list.ts';
...
export function setInt64(byteOffset: number, value: bigint, s: Struct, defaultMask?: DataView): void {
    ...
    if (defaultMask !== undefined) {
        ...
        const lo = TMP_WORD.getUint32(NATIVE_LITTLE_ENDIAN ? 0 : 4, NATIVE_LITTLE_ENDIAN) ^
            defaultMask.getUint32(0, true);
        const hi = TMP_WORD.getUint32(NATIVE_LITTLE_ENDIAN ? 4 : 0, NATIVE_LITTLE_ENDIAN) ^
