...
export function generateCapnpImport(ctx: CodeGeneratorFileContext): void {
    ...
    const tsImportPathAnnotation = tsAnnotationFile &&
    ...
    const importAnnotation = tsImportPathAnnotation &&
        fileNode.getAnnotations().find((a) => a.getId() === tsImportPathAnnotation.getId());
    const importPath = importAnnotation === undefined ? '@jkomyno/capnp-ts' : importAnnotation.getValue().getText();
    ...
    ctx.statements.push(
        f.createExpressionStatement(
            f.createIdentifier(`import { ObjectSize as __O, Struct as __S } from '${importPath}'`),
        ),
    );
}
...
export function generateNestedImports(ctx: CodeGeneratorFileContext): void {
    ctx.imports.forEach((i) => {
        ...
        if (name.substring(0, 7) === '/capnp/') {
            importPath = `capnp-ts/src/std/${name.substring(7)}.js`;
        } else {
...
export function generateStructFieldMethods(
    ...
    if (has) {
        ...
        const expressions = [
            f.createLogicalNot(
                f.createCallExpression(f.createPropertyAccessExpression(STRUCT, 'isNull'), __, [getPointer]),
            ),
        ];
